 ...
 *
 * 需要传递的属性
 * 1. afterUploadHandle = (url, category, filename) = {}: 得到图片链接后需要的处理函数
 * @param {*} props
 */
export const UploadImageTabs = (props) => {
  ...
  const handleUploadImage = useCallback(() => {
    ...
    for (var i = 0; i < fileListData.length; i++) {
      ...
      var fileType = fileListData[i].type;
    ...
    // 2020-12-19：调整了上传接口
    let url = "/api/v1/storage/file/upload";
    fetchApi
      .Post(url, formData, {})
      .then((responseData) => {
        ...
        if (responseData.fileurl) {
          if (props.afterUploadHandle) {
            props.afterUploadHandle(responseData.fileurl, responseData.category, responseData.filename);
          } else {
            console.log("未传递：afterUploadHandle");
          }
        } else if (responseData.file) {
          if (props.afterUploadHandle) {
            props.afterUploadHandle(responseData.file, responseData.category, responseData.filename);
          } else {
            console.log("未传递：afterUploadHandle");
          }
        } else {
  ...
  const imageUrlChoicesElements = useMemo(() => {
    if (props.imageUrls && Array.isArray(props.imageUrls)) {
      return props.imageUrls.map((item, index) => {
        return (
          <div
            className={imageUrl === item ? "image-item active" : "image-item"}
            key={index}
            onClick={(e) => {
              e.stopPropagation();
              setImageUrl(item);
  ...
  return (
    <div className="upload-image-tabs">
      ...
      >
        <Tabs.TabPane tab="上传图片/文件" key="uploadImage">
        ...
        {props.disableLink || (
          <Tabs.TabPane tab="使用链接" key="useLink">
            ...
            {/* 图片的选项 */}
            <div className="image-choices">{imageUrlChoicesElements}</div>
            {activeTabKey === "useLink" &&
              imageUrl &&
              checkImageUrlPattern.test(imageUrl) && (
      ...
      <div className="button">
        <Button type="primary" onClick={handleSubmit} disabled={submitDisable}>
          提交
        </Button>
      </div>
    </div>
...
export const UploadImageTabsModal = (props) => {
  ...
  const handleOnCloseOrOk = useCallback(
    (e) => {
      // console.log(e);
